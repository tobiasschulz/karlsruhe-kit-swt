...
public class Mosaique {
	...
	public void process() throws IOException, ClassNotFoundException {
		...
		// create the thread objects in an array list
		final int numThreads = config.getNumThreads();
		ArrayList<MosaiqueThread> threads = new ArrayList<MosaiqueThread>();
		for (int i = 0; i < numThreads; ++i) {
			threads.add(new MosaiqueThread(this));
		}

		// assign the tiles to the threads
		for (int c = 0; c < numCols; c++) {
			for (int r = 0; r < numRows; r++) {
				final int numTile = c * numRows + r;
				threads.get(numTile % numThreads).addTileToDo(
						new Dimension(c, r));
			}
		}
		System.out.println("Number of Threads: " + threads.size());
		System.out.println("Number of Tiles: " + (numCols * numRows));
	...
	/**
	 * @return the myClassifiedDirectory
	 */
	public ClassifiedDirectory getClassifiedDirectory() {
		return myClassifiedDirectory;
	}
