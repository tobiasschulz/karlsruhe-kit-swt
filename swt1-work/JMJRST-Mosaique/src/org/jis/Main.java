...
public class Main extends JFrame {
	...
	private void init() {
		...
		try {
			jOutputDoc.insertString(jOutputDoc.getLength(),
					mes.getString("Main.2") + Options.ls, readyAtr);
		...
		JSplitPane obenrechts = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				list, preview);
		obenrechts.setContinuousLayout(true);
		obenrechts.setOneTouchExpandable(true);
		obenrechts.setResizeWeight(0.6D);

		JSplitPane oben = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, jsp2,
				obenrechts);
	...
	public static void main(String[] args) {
		if (args.length == 0)
			...
			if (args.length == 1
					&& args[0].equalsIgnoreCase(mes.getString("Main.3"))) {
				System.out.println(System.getProperty("line.separator")
						+ mes.getString("Main.5")
						+ System.getProperty("line.separator")
						+ mes.getString("Main.7"));
			...
			for (int i = 0; i < args.length; i++) {
				...
				try {
					if (args[i].substring(0, 4).equalsIgnoreCase(
							mes.getString("hmax")))
						hmax = args[i].substring(
								args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 4).equalsIgnoreCase(
							mes.getString("vmax")))
						vmax = args[i].substring(
								args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 5).equalsIgnoreCase(
							mes.getString("input")))
						input = args[i].substring(
								args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 6).equalsIgnoreCase(
							mes.getString("output")))
						output = args[i].substring(
								args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
					if (args[i].substring(0, 7).equalsIgnoreCase(
							mes.getString("quality")))
						quality = args[i].substring(
								args[i].lastIndexOf(mes.getString("=")) + 1,
								args[i].length()).trim();
				} catch (Exception e) {
				}
			...
			System.out
					.println(mes.getString("Main.30") + fi.toString() + mes.getString("Main.31") + fo.toString() + mes.getString("Main.32") + q); //$NON-NLS-3$
	...
	public void setLookFeel(String look) {
		try {
			...
			try {
				jOutputDoc.insertString(jOutputDoc.getLength(),
						mes.getString("Main.33") + look + Options.ls, errorAtr);
				text.setCaretPosition(jOutputDoc.getLength());
			} catch (Exception e) {
