...
import org.jis.Main;
import org.jis.mosaique.MosaiqueDialog;
...
public class MenuListner implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == menu.info) {
			new AboutBox(m);
			System.gc();
		} else if (e.getSource() == menu.gener
				|| e.getSource() == m.toolBar.gener)
			m.generator.generate(false);
		else if (e.getSource() == menu.zippen
				|| e.getSource() == m.toolBar.zippen)
			m.generator.generate(true);
		else if (e.getSource() == menu.gallerie
				|| e.getSource() == m.toolBar.gallerie)
			new GallerieDialog(m);
		else if (e.getSource() == menu.mosaik || e.getSource() == m.toolBar.mosaik)
			new MosaiqueDialog(m);
		else if (e.getSource() == menu.exit)
			System.exit(0);
		else if (e.getSource() == menu.set_quality
				|| e.getSource() == m.toolBar.preferences)
			m.openOptions();
			...
			try {
				URL url = new URL(
						"http://www.jgeppert.com/jmjrst/jmjrst_version.txt");
				...
				if (line != null) {
					...
					if (!line.equalsIgnoreCase(m.mes.getString("Version"))) {
						...
						JOptionPane.showMessageDialog(m, tf,
								m.mes.getString("Menu.15"),
