...
public class GallerieDialog extends JDialog {
	...
	public GallerieDialog(Main m) throws HeadlessException {
		...
		String types[] = { m.mes.getString("GallerieDialog.7"),
				m.mes.getString("GallerieDialog.8") };
		...
		URL url = ClassLoader
				.getSystemResource("icons/applications-graphics.png");
					...
					public void run() {
						try {
							if (getSelectedLayout().getPreviewImage() != null)
								setPreview(new Thumbnail(getSelectedLayout()
										.getPreviewImage(), 300, 240));
							else
							...
							System.out
									.println("Fehler beim erstellen der Vorschau!");
							ex.printStackTrace();
						}
					}
		...
		p_color.setBackground(new Color(o.getBackground_r(), o
				.getBackground_g(), o.getBackground_b()));
		p_t_color.setBackground(new Color(o.getForeground_r(), o
				.getForeground_g(), o.getForeground_b()));
				...
				if (files[i].isDirectory()) {
					...
					if (settings.exists() && settings.isFile()
						...
						try {
							...
							x.setSmallWidth(Integer.parseInt(p
									.getProperty("smallWidth")));
							x.setSmallHeight(Integer.parseInt(p
									.getProperty("smallHeight")));
							x.setMediumWidth(Integer.parseInt(p
									.getProperty("mediumWidth")));
							x.setMediumHeight(Integer.parseInt(p
									.getProperty("mediumHeight")));
							x.setBigWidth(Integer.parseInt(p
									.getProperty("bigWidth")));
							x.setBigHeight(Integer.parseInt(p
									.getProperty("bigHeight")));

							if (p.getProperty("mediumCreate").equalsIgnoreCase(
									"true"))
								x.setMediumCreate(true);
							else
								x.setMediumCreate(false);

							if (p.getProperty("bigCreate").equalsIgnoreCase(
									"true"))
								x.setBigCreate(true);
							else
								x.setBigCreate(false);

							if (p.getProperty("subTitle").equalsIgnoreCase(
									"true"))
								x.setSubTitle(true);
							else
								x.setSubTitle(false);

							x.setMax_pictures_on_site(Integer.parseInt(p
									.getProperty("max_pictures_on_site")));
							x.setFile(files[i]);
	...
	ActionListener al = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == b_exit)
				...
				Color c = JColorChooser.showDialog(gallerieDialog,
						m.mes.getString("GallerieDialog.5"), Color.DARK_GRAY);
				if (c != null) {
				...
				Color c = JColorChooser.showDialog(gallerieDialog,
						m.mes.getString("GallerieDialog.6"), Color.DARK_GRAY);
				if (c != null) {
				...
				o.setGallerieWidth(Integer.parseInt(c_tab_width
						.getSelectedItem().toString()));
				o.setGallerieHeigth(Integer.parseInt(c_tab_heigth
						.getSelectedItem().toString()));
